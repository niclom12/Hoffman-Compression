...
{
    /* Sort SA by rank[i + k] (second key) using counting sort.  
       rank[>=n] is treated as 0. */
    int maxv = (n > 256) ? n : 256;           /* ranks are in 0..max(n,255) */
    ...
    for (int i = 1; i <= maxv; ++i)           /* prefix sums */
        cnt[i] += cnt[i - 1];
...
size_t bwt_encode2(const uint8_t *T, size_t n, uint8_t *L)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        sa[i]   = (int)i;
        rank[i] = S[i];                       /* initial ranks = byte value */
    }

    for (int k = 1; k < (int)N; k <<= 1) {
        /* radix sort on (rank[i], rank[i+k]) – second key, then first key */
        ...
        if (rank[sa[N - 1]] == (int)N) break;    /* all ranks unique */
    }

    /* ----- build BWT last column & find primary ------------------------ */
    size_t primary = 0;
    for (size_t i = 0; i < N; ++i) {
        int j = sa[i];
        L[i]  = (uint8_t)(j ? S[j - 1] : S[N - 1]);  /* char before suffix */
        if (j == 0) primary = i;
    }
