...
int compress(const char *src, const char *dst, int use_bwt, int use_mtf, int use_rle)
{
    ...
    size_t len = n;
    size_t primary = 0; /* only set if we run BWT */

    /* ---------- BWT ---------- */
    if (use_bwt)
    {
        uint8_t *bwt_buf = malloc(len + 1);
        primary = bwt_encode2(buf, len, bwt_buf);
        if (buf != (unsigned char *)orig)
            free(buf);
        buf = bwt_buf;
        len += 1;
    }
    ...
    huff_encode((unsigned char *)buf, len, hlen, hcode);
    free(buf);
...
int decompress(const char *src, const char *dst, int use_bwt, int use_mtf, int use_rle)
{
    ...
    unsigned char *buf;
    size_t len;
    uint8_t flags = (use_bwt ? 1 : 0) | (use_mtf ? 2 : 0) | (use_rle ? 4 : 0);
    ...
    /* ---- Huffman (always) ---- */
    len = huff_decode(bitbuf, bit_len, hlen, hcode, NULL);
    buf = malloc(len);
    huff_decode(bitbuf, bit_len, hlen, hcode, buf);
    ...
    if (flags & 1)
    {
        uint8_t *tmp = bwt_decode(buf, len, primary);
        free(buf);
        buf = tmp; /* len == orig_len now */
    }
